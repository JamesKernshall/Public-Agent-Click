...
bool GamePlay::init()
{
	...
	background_music.openFromFile("Data/Audio/JazzLoop.mp3");
...
void GamePlay::update(float dt)
{
	...
	{
		if (!BasicInteractable->isInteractable)
		{
			continue;
		}
...
void GamePlay::mousePressed(sf::Event event)
{
...
void GamePlay::OnMouseClick(sf::Event event)
{
	sf::Vector2f mouse_pos = Data::GetInstance()->window->mapPixelToCoords(sf::Mouse::getPosition(*Data::GetInstance()->window));
...
void GamePlay::EditorKeyPreciseMoveObjects(sf::Event event)
{
	// Saving
...
void GamePlay::EditorMousePlaceObjects(sf::Event event)
{
	sf::Vector2f mouse_pos = Data::GetInstance()->window->mapPixelToCoords(sf::Mouse::getPosition(*Data::GetInstance()->window));
